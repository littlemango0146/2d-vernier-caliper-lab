(() => {
  ...
  function makeMainScale(group) {
    group.append(svgEl("line", { x1: SCALE_X, y1: 225, x2: SCALE_X + MAX_MM * PX_PER_MM, y2: 225, stroke: "#52637a", "stroke-width": 2 }));
    ...
    group.append(svgEl("text", { x: SCALE_X + MAX_MM * PX_PER_MM + 45, y: 132, fill: "#66758a", "font-size": 17, "font-family": "Cascadia Mono, monospace" }, "mm"));
  }
  ...
  function makeVernier(group) {
    ...
    group.append(svgEl("rect", { x: -28, y: 195, width: Math.max(bodyWidth + 40, 150), height: 335, fill: "transparent", class: "slider-hit", "data-slider-hit": "true", tabindex: 0, role: "slider", "aria-label": "活動量爪位置", "aria-valuemin": 0, "aria-valuemax": MAX_MM, "aria-valuenow": valueMm().toFixed(2) }));
    return bodyWidth;
  }
  ...
  function makeWorkpiece() {
    ...
    if (state.workpieceMode === "outer") {
      group.append(svgEl("rect", { x: SCALE_X + 3, y: 344, width: Math.max(12, targetWidth - 6), height: 158, rx: 18, fill: workFill, stroke: workStroke, "stroke-width": contact ? 5 : 3 }));
      ...
      group.append(svgEl("text", { x: SCALE_X + targetWidth/2, y: 416, fill: "#8a5427", "font-size": 15, "text-anchor": "middle", "font-family": "Cascadia Mono, monospace", "font-weight": 700 }, "未知外徑"));
    } else {
  ...
  function renderCaliper() {
    ...
    bg.append(svgEl("path", {
      d: "M 160 78 H 245 L 386 243 H 245 V 512 H 130 V 243 H 90 V 78 Z",
      fill: "#c8d5e5", stroke: "#8fa0b6", "stroke-width": 3,
    ...
    bg.append(svgEl("path", { d: "M 195 78 H 230 V 17 H 212 L 192 44 Z", fill: "#c8d5e5", stroke: "#8fa0b6", "stroke-width": 3 }));
  ...
  function updateViewBox() {
    ...
    if (!mobile && state.viewZoom === 1) {
      ...
      $("#scaleWindowLabel").textContent = "完整尺身 · 0—150 mm";
      return;
    }
    const baseWidth = mobile ? 560 : FULL_WIDTH / state.viewZoom;
    ...
    const width = mobile ? baseWidth : Math.min(FULL_WIDTH, Math.max(baseWidth, workpieceViewWidth));
    const zeroX = SCALE_X + valueMm() * PX_PER_MM;
    const desiredStart = hasWorkpiece() && !mobile ? SCALE_X - 120 : zeroX - width * .3;
    const start = Math.max(0, Math.min(FULL_WIDTH - width, desiredStart));
    caliperSvg.setAttribute("viewBox", `${start} ${mobile ? 70 : 0} ${width} ${mobile ? 490 : 560}`);
    const firstMm = Math.max(0, Math.floor((start - SCALE_X) / PX_PER_MM));
    const lastMm = Math.min(MAX_MM, Math.ceil((start + width - SCALE_X) / PX_PER_MM));
    $("#scaleWindowLabel").textContent = `${mobile ? "游標跟隨" : `刻度放大 ${state.viewZoom}×`} · ${firstMm}—${lastMm} mm`;
  }
  ...
  function updateUI() {
    ...
    $("#positionOutput").textContent = `${fmt(value)} mm`;
    ...
    $("#readingDisplay").hidden = state.mode === "quiz";
    ...
    $("#stepResult").innerHTML = `<span>STEP ${String(Math.max(1, state.readingStep)).padStart(2, "0")}</span><p>${safeText(stepText)}</p>`;
  }
  ...
  window.addEventListener("keydown", event => {
    if (event.key.toLowerCase() === "r" && !event.target.closest("input, button, dialog")) setStep(0);
